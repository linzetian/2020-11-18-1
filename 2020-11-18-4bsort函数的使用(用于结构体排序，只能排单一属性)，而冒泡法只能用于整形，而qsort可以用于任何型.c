...
{
	return strcmp(((struct stu*)p1)->name ,((struct stu*)p2)->name);//因为p1为void*指针所以强制转换为float*，指针类型不同访问的空间不一样,两个字符串不能相减进去字符串比较
}
void test3()
{
	int i = 0;
	...
	for (i = 0; i < sz; i++)
		printf("%s ", s[i].name);//为什么用是s[i]->age会报错，因为->前面要为指针，需要定义s为结构体指针，然后就可以用->,输出为字符串（s）
}
int main()
{
